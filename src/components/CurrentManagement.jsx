...
const CurrentManagement = () => {
  ...
  const validateForm = () => {
    ...
    if (!newItem.stock || isNaN(newItem.stock)) errors.stock = 'Valid stock is required';
    if (!newItem.price || isNaN(newItem.price)) errors.price = 'Valid price is required';
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      <h1 className="text-2xl font-bold mb-4">Current Management</h1>
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id} className="border-t">
                  ...
                  <td className={`px-4 py-2 ${item.stock < 10 ? 'text-red-500' : ''}`}>
                    {item.stock}
                  </td>
