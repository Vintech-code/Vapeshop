...
const PurchaseHistory = () => {
  ...
  const filteredPurchases = purchases.filter(purchase => {
    try {
      if (!purchase || !purchase.user) return false;
      
      const searchLower = searchTerm.toLowerCase();
      return (
        (purchase.user.name && purchase.user.name.toLowerCase().includes(searchLower)) ||
        (purchase.payment_method && purchase.payment_method.toLowerCase().includes(searchLower)) ||
        (purchase.total_amount && purchase.total_amount.toString().includes(searchTerm)) ||
        (purchase.created_at && formatDate(purchase.created_at).toLowerCase().includes(searchLower))
      );
    } catch (error) {
      console.error('Error filtering purchase:', error, purchase);
      return false;
    }
  });
